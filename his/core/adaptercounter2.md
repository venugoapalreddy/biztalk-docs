 ...
 **count**  
 ...
 <strong>type</strong>  
 The event type that is being monitored with this <strong>ADAPTERCOUNTER</strong>. The <strong>type</strong> member instructs Perfmon whether the <strong>count</strong> member represents a numeric counter such as number of connection failures, a rate such as throughput in bytes transferred per second, or a percentage. For suitable values, see the platform SDK documentation of <strong>PERF_COUNTER_\</strong>* (for example, <strong>PERF_COUNTER_COUNTER</strong> or <strong>PERF_COUNTER_RAWCOUNT</strong>).  
