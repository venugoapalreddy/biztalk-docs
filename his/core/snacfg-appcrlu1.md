 ...
 **/list**  
 ...
 <em>connectionname</em> <strong>:</strong> <em>LUalias</em>  
 Specifies the connection and LU alias of the remote APPC LU on which to carry out actions. You should include <em>connectionname</em><strong>:</strong> in <strong>snacfg appcrlu</strong> commands (other than <strong>/add</strong> commands) that include <em>LUalias</em>. Without <em>connectionname</em><strong>:</strong>, if there is more than one remote LU called <em>LUalias</em> in the subdomain, it is difficult to predict which of these LUs will be affected by the command. The <strong>snacfg appcrlu</strong> command does not necessarily default to a connection on the local server if <em>connectionname</em> is omitted.  
 ...
 **/netname:**" *text*"  
 ...
 If <strong>/netname:</strong><em>text</em> is not specified, the remote network name configured for the connection supporting this LU is used as the default.  
 ...
 **/impmode:** *modename*  
 ...
 <strong>/security:{ none</strong>&#124; <strong>hex,</strong><em>text</em>&#124; <strong>char,</strong><em>text</em> <strong>}</strong>  
 Configures session security for a remote LU using a cleartext key. The <strong>none</strong> option turns off session-level security. The <strong>hex,</strong><em>text</em> option specifies a 16-digit security key in hexadecimal. The <strong>char,</strong><em>text</em> option specifies an eight-character security key that can include uppercase and lowercase alphanumeric characters, and the special characters $, @, #, and the period (.).  
 ...
 **/delpartner:** *LUalias* **,** *mode*  
 ...
 <strong>/securityex:{ none</strong>&#124; <strong>hex,</strong><em>text</em>&#124; <strong>char,</strong><em>text</em> <strong>}</strong>  
