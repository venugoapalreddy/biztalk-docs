...
- Special characters $, #, @, period, and space.  
  ...
  For example, if you supply the underscore (<em>) character (ASCII value F6) while creating an ASCII to EBCDIC conversion table, the character that appears in the converted strings will be 6, the character associated with the value F6 in EBCDIC. To use the \</em> character as the substitute character in an ASCII to EBCDIC conversion table, you should supply the value E1 (the value associated with the \_ character in EBCDIC) rather than the actual character.  
