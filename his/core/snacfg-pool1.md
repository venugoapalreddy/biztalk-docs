...
## Purpose  
 ...
 To assign existing 3270 LUs to a 3270 LU pool, first configure the pool with the <strong>snacfg pool</strong> command (including options), then add the LUs with the <strong>snacfg lu</strong> command (using the <strong>/pool:</strong><em>poolname</em> option).  
