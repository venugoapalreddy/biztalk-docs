...
9. In Solution Explorer, right-click <strong>WSClient</strong>, and then click <strong>Build</strong>. Ensure that a success message appears in the output window. If no success message appears, troubleshoot <strong>WSClient</strong>. [!INCLUDE [btsVStudioNoVersion](../../includes/btsvstudionoversion-md.md)] places a copy of the executable, WSClient.exe, into the \<<em>drive</em>\>:\Tutorial\WSClient\bin\Debug folder.  
