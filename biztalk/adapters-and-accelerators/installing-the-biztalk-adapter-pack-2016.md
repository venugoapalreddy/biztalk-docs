...
1. Run the [!INCLUDE [btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] <strong>setup.exe</strong>.  
...
2. Select **Install Microsoft BizTalk Adapters**. In the next window, any missing prerequisite programs are listed. If any are missing, then select the missing program, and setup installs it for you. 
   ...
   > [!NOTE]
   >  If you are installing the [!INCLUDE [adapterpacknoversion](../includes/adapterpacknoversion-md.md)] on a virtual machine, the setup wizard may display a message that it's checking for available disk space. If this message appears to hang or just sit there, then we recommend you <strong>Install in silent mode</strong> (in this topic).  
