...
- The <strong>Message publishing incoming rate</strong> for the host instance exceeds the <strong>Message publishing outgoing rate\</strong>* the specified <strong>Rate overdrive factor</strong> value. The <strong>Rate overdrive factor</strong> value is defined in <strong>Settings Dashboard</strong>, <strong>Hosts</strong> and then <strong>Rate-Based Throttling</strong>. The message publishing incoming and outgoing rates can be measured with the corresponding performance monitor counters under the <strong>BizTalk:Message Agent</strong> performance object category.  
