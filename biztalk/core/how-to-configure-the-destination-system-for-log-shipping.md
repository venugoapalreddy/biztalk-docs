...
7. Select **New Query**. In the query window, paste the following command:  
   ...
   > [!NOTE]
   >  If you have more than one source server, you can restore each source server to its own destination server. On each destination server, in the <strong><xref href="SourceServerName" data-throw-if-not-resolved="False" data-raw-source="@SourceServerName"></xref> = null</strong> parameter, replace <em>null</em> with the name of the appropriate source server, surrounded by single quotes (for example, <strong><xref href="SourceServerName" data-throw-if-not-resolved="False" data-raw-source="@SourceServerName"></xref> = 'MySourceServer',</strong>).  
...
12. If you have more than one MessageBox database in your [!INCLUDE [btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] system, add another MessageBoxDB line to the list, and then set <strong>IsMaster="0"</strong> for the non-master databases.  
